...
return FPDFAnnot_SetFormFieldFlags(handle,annot,flags);
}
...
return FPDFAnnot_SetFontColor(handle,annot,R,G,B);
}
...
return FPDFAnnot_GetFontColor(hHandle,annot,R,G,B);
}
...
return FPDF_StructElement_Attr_GetBooleanValue(value,out_value);
}
...
return FPDF_StructElement_Attr_GetNumberValue(value,out_value);
}
...
return FPDF_StructElement_Attr_GetStringValue(value,buffer,buflen,out_buflen);
}
...
return FPDF_StructElement_Attr_GetBlobValue(value,buffer,buflen,out_buflen);
}
...
return FPDF_StructElement_Attr_GetChildAtIndex(value,index);
}
...
return FPDFCatalog_SetLanguage(document,language);
}
...
return FPDFPage_InsertObjectAtIndex(page,page_object,index);
}
...
return FPDFPageObj_TransformF(page_object,matrix);
}
...
return FPDFPageObj_GetMarkedContentID(page_object);
}
...
return FPDFImageObj_GetIccProfileDataDecoded(image_object,page,buffer,buflen,out_buflen);
}
...
return FPDFText_LoadCidType2Font(document,font_data,font_data_size,to_unicode_cmap,cid_to_gid_map_data,cid_to_gid_map_data_size);
}
...
return FPDFFont_GetFamilyName(font,buffer,length);
}
...
return FPDFFormObj_RemoveObject(form_object,page_object);
}
...
return FPDF_SetSystemFontInfo(font_info);
}
...
return FPDF_FreeDefaultSystemFontInfo(font_info);
}
...
return FPDFText_GetTextObject(text_page,index);
}
...
return FPDFAttachment_GetSubtype(attachment,buffer,buflen);
}
